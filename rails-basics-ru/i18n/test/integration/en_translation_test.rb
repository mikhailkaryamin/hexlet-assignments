...
class EnTranslationTest < ActionDispatch::IntegrationTest
  ...
  test 'posts#index' do
    ...
    assert_select '#new-post-link', text: 'Add post'
  end
  ...
  test 'posts#new' do
    ...
    assert_select '.new_post #post_title[placeholder=?]', 'Title'
  end
  ...
  test 'posts#create check validation' do
    ...
    assert_select '.new_post .post_title .invalid-feedback', "Title can't be blank"
  end
  ...
  test 'posts#destroy' do
    ...
    assert_select '.alert-info', 'Post was destroy.'
  end
